...
cd /data/ooni/private/canned && for date in 2017-07-{01..30}; do
    echo -n "$date -- "
    {
    cat $date/index.json | jq 'select(.canned != null) | .canned[] | .textname + " " + (.text_size | tostring)'
    cat $date/index.json | jq 'select(.canned == null) | .textname + " " + (.text_size | tostring)'
    } | sed 's,.*/,,; s,"$,,' | LC_ALL=C sort | sha256sum
done |& less

# at chameleon.infra.ooni.io
cd /data/ooni/private/reports-raw/yaml && for date in 2017-07-{01..30}; do
