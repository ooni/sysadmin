...
-t nat -I PREROUTING -i eth0 -s {{ lookup('dig', 'hkgmetadb.infra.ooni.io/A') }} -p udp --dport 1194 -j DNAT --to-destination {{ lookup('dig', 'amsmetadb.ooni.nu/A') }}
-t nat -I POSTROUTING -o eth0 -s {{ lookup('dig', 'hkgmetadb.infra.ooni.io/A') }} -d {{ lookup('dig', 'amsmetadb.ooni.nu/A') }} -p udp --dport 1194 -j SNAT --to-source {{ lookup('dig', 'mia-ps-test.ooni.nu/A') }}
